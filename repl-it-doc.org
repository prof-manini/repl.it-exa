...
* Version control
  ...
  Uno strumento indispensabile nello sviluppo software (e non solo) è
  un *sistema di controllo di revisione* (o di *versione*), di solito
  abbreviato in =rcs= (*revision control system*)= o =vcs= (*version ...*).

  Gli *rcs* sono sistemi decisamente *complessi* che agevolano il lavoro
  "individuale", permettono di collaborare con altre persone, e
  offrono anche un sistema di rete che permette di accedere al codice
  anche "in remoto". Fortunatamente si rivelano utili anche se si
  conoscono solo i concetti di base e si utilizzano solo alcune
  funzionalità.

  La situazione in cui ci poniamo è la seguente:

  1. abbiamo un progetto, ossia un lavoro in corso che produce un
     qualche risultato (spesso chiamato *deliverable*), ad esempio un
     software di qualche tipo: un esercizio di Python o di SQL,

  2. abbiamo una *working directory* (cartella di lavoro) che contiene
     *tutti* i file che vogliamo gestire: codice sorgente, dati di
     esempio, documentazione, il *prodotto finale* etc;

  3. il progetto si sviluppa e cambia nel tempo e lo stesso succede al
     contenuto della /working directory/ in quanto, usando vari
     strumenti (editor di testo e quant'altro) abbiamo creato,
     modifichiamo, rinominato, cancellato cartelle e file,

  4. ogni volta che cambiamo qualcosa la "situazione precedente" viene
     dimenticata; quando salvo un file il nuovo contenuto *sostituisce*
     quello precedente,

  5. morale: non mantengo uno *storico* del mio progetto e mi troverò
     sempre a dire "ma prima funzionava".

  Per questi ed altri motivi serve un sistema che mi permetta si
  salvare delle *istantanee* (degli "*screenhsot*") della mia /working
  directory/ e di poter "tornare indietro nel tempo" recuperando un
  qualsiasi stato precedente.

** Vocabolario minimo

   - working directory :: come già detto quando si lavora "in locale"
        (sul proprio computer) la working directory è una cartella
        come tutte le altre, in =repl.it= è il =repl= che abbiamo creato a
        inizio sessione e che dal quale possiamo poi ripartire,

   - commit :: usato come verbo (/to commit/) è l'azione di "salvare" lo
               stato attuale della /working directory/ nel /repository/,
               usato come sostantivo indica ciascun singolo
               salvataggio,

   - repository :: è il "contenitore" in cui l'=rcs= salva le varie
                   istantanee più un sacco di altre informazione
                   (metadati) su tutto quello che succede (un =rcs= non
                   dimentica e non cancella mai nulla!).

   - checkout :: è l'azione di "estrarre" una delle versioni
                 precedentemente salvate nel /repository/; da fare con
                 attenzione perché modifica il contenuto della
                 /working directory/ e quindi si rischia di perdere
                 delle modifiche se queste non sono state "salvate" in
                 un commit!

** git e github

   [[https://en.wikipedia.org/wiki/Git][git]] è un =rcs= scritto da [[https://en.wikipedia.org/wiki/Linus_Torvalds][Linus Torvalds]] nel 2005 per supportare lo
   sviluppo del [[https://en.wikipedia.org/wiki/Linux_kernel][kernel Linux]], è un software "free and open-source"
   ([[https://en.wikipedia.org/wiki/Free_and_open-source_software][FOSS]]) un progetto e ora uno degli =rsc= più utilizzati.  =git= è anche
   la "base" del servizio on-line [[https://github.com/][github]] che è forse (?) il più grande
   "repository" (magazzino?) di software al mondo (ospita anche tutto
   lo sviluppo di Python e di SQLite).

** Cosa offre repl.it

   =repl.it= usa =git= (=github=?) per offrire un servizio di controllo di
   revisione minimale ma molto semplice da usare, più che sufficiente
   per i nostri scopi.

   Supponiamo di avere già scritto un po' di codice, ad esempio nel
   solito =main.py=.  Cliccando il secondo (di sei) pulsante nella parte
   sinistra della pagina viene mostrato un pulsante con scritto *Create
   a git repo*.  Cliccando questo nuovo pulsante appare, tra le altre
   cose, un rettangolo con scritto *Initial commit*, che indica che è
   stata "salvata" una prima versione del progetto.

   Se adesso modifico il sorgente, lo "spazio" a sinistra cambia e
   appare un'area di testo con scritto *What did you change* e un
   pulsante *Commit and push*.  Se inserisco un commento sulle modifiche
   appena fatte e premo il pulsante viene creato un nuovo *commit* in
   cui viene salvata la versione corrente.

   Adesso, nella parte sinistra ci sono *due* blocchi, ciascuno con un
   pulsante *Revert to here* che permette di "ritornare lì" facendo un
   *check out* di quella specifica versione.
...
: 3,Bob,666-B,63
    ...
    primo tra tutti il fatto che non si può sapere in anticipo quante
    saranno le colonne, quali "nomi" avranno e in che ordine saranno
    (a meno di non controllare la relativa =create=).  Ricordarsi che le
    tabelle di una tabella possono cambiare nel tempo!  È quindi molto
    meglio esplicitare sempre la lista delle colonne.
...
**** Collegamento (full join)

     Questa sezione è forse un po' troppo tecnica, ma può essere
     interessante per capire come la =join via where= dell'esempio
     precedente produce (o possa produrre) il risultato voluto.

     Quando nella =from= di una =select= sono presenti due tabelle, l'SQL
     produce una riga per ciascuna combinazione (coppia) possibile
     delle righe delle due tabelle.  Nell'esempio ho tre nomi e cinque
     numeri e quindi ottengo quindici righe.  Non tutte queste righe
     però "hanno senso", perché per esempio ci sono anche quelle di
     =Linus= con il numero di telefono di =Richard= o di =Bob=.  Queste però
     vengono eliminate dalla =where=, che richiede che in ciascuna riga
     =Person.id= abbia lo stesso valore di =Phone.id_person= ossia che
     venga utilizzato il collegamento tra chiave primaria di =Person= e
     chiave esterna di =Phone=.

     Nell'esempio che segue si vede il risultato della =join= con tutte
     le righe; l'ultima colonna ha valore =1= se chiave primaria e
     chiave esterna coincidono ossia se la riga è una di quelle
     "buone".

     *Nota*: l'ultima colonna non contiene quindi il valore di un *campo*
     di una delle due tabella, ma, come spesso succede, un valore
     *calcolato* a partire dai dati delle due tabelle. Questo è il
     motivo per cui, ad un certo punto, abbiamo parlato di *descrittori*
     di colonne.

     *Nota*: in questo esempio ho usato l'operatore =as= che permette di
     assegnare un nome a piacere a una colonna.  Questa operatore è
     indispensabile quando ci sono più colonne (ovviamente di tabelle
     diverse) con lo stesso nome, o quando si desidera un nome più
     espressivo. Si usa molto frequentemente anche per abbreviare,
     spesso alla sola iniziale, i nomi delle tabelle quando questi
     compaiono più volte nella stessa =select=.
...
.mode column
.width 5 5
select Person.id as PK, Phone.id_person as FK,
       name, number,
       Person.id = Phone.id_person as "PK = FK"
from Person, Phone;
#+end_src

#+RESULTS: phone-full-join
#+begin_example
PK     FK     name        number         PK = FK
-----  -----  ----------  -------------  ----------
1      1      Linus       123-44-55-666  1
1      1      Linus       123-44-00-888  1
1      2      Linus       300-00-11-22   0
1      2      Linus       300-00-33-00   0
1      3      Linus       Bob,400-99-55  0
2      1      Richard     123-44-55-666  0
2      1      Richard     123-44-00-888  0
2      2      Richard     300-00-11-22   1
2      2      Richard     300-00-33-00   1
2      3      Richard     Bob,400-99-55  0
3      1      Bob         123-44-55-666  0
3      1      Bob         123-44-00-888  0
3      2      Bob         300-00-11-22   0
3      2      Bob         300-00-33-00   0
3      3      Bob         Bob,400-99-55  1
#+end_example
